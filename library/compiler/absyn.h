...
namespace Absyn {
  ...
  typedef struct PrimopOpt *primop_opt_t;
...
#if 0
  typedef datatype ConstraintVar @cvar_t,
                                 *cvar_opt_t;
#endif
  ...
  // for integral types
  EXTERN_ABSYN enum Sign {
    Signed,
    Unsigned,
    None,
  };

  EXTERN_ABSYN enum AggrKind {
    StructA,
    UnionA,
  };
  ...
  EXTERN_ABSYN datatype EffConstraint {
    SingleConstraint(type_t);  // single(e1) used to ensure that a pointer *points into* a given
                               // region, e.g in rfree
  ...
  EXTERN_ABSYN enum Format_Type {
    Printf_ft,
    Scanf_ft,
  };

  EXTERN_ABSYN enum Access_Type {
    WriteOnly_at,
  };
  ...
  EXTERN_ABSYN datatype Attribute {
    ...
    Leaf_att;
    Malloc_att;
    Access_att(enum Access_Type,
               int,         // buffer arg
               int);        // size arg
    Cold_att;
    AllocSize_att(int, int);
    AllocAlign_att(int);
  ...
  // optional primop for assignment operators
  EXTERN_ABSYN struct PrimopOpt {
    primop_t v;
  };

  // ++x, x++, --x, x-- respectively
  EXTERN_ABSYN enum Incrementor {
    PreInc,
    PostInc,
    PreDec,
    PostDec,
  ...
  EXTERN_ABSYN datatype Raw_exp {
    ...
    AssignOp_e(exp_t, primop_opt_t, exp_t);     // e1 = e2, e1 += e2, etc.
    ...
    Address_e(exp_t);                         // &e
    New_e(exp_opt_t, exp_t, exp_opt_t);       // first expression is region -- null is heap, 3rd is
                                              // aqual_t -- null is ALIASABLE
    ...
    CompoundLit_e($(var_opt_t, tqual_t, type_t) @, list_t<$(list_t<designator_t>, exp_t) @>);
    ...
    Aggregate_e(typedef_name_t, types_t,  // witness types, maybe fewer before type-checking
                list_t<$(list_t<designator_t>, exp_t) @>, struct Aggrdecl *);
    // { .x1 = e1, ...., .xn = en }
    ...
    UnresolvedMem_e(opt_t<typedef_name_t>, list_t<$(list_t<designator_t>, exp_t) @>);
    StmtExp_e(stmt_t);  // ({s;e})
    Tagcheck_e(exp_t, field_name_t);
    // the type must have IntKind.  Valueof_e can only
    ...
    Asm_e(bool,                          // volatile
          string_t,                      // instr
          list_t<$(string_t, exp_t) @>,  // : outvars
          list_t<$(string_t, exp_t) @>,  // : invars
          list_t<string_t @>);           // : clobbers
    ...
    // a static assertion that shouldn't be proved by the analysis
    Assert_false_e(exp_t);
  };
  ...
  EXTERN_ABSYN datatype Raw_pat {
...
#if 0
    Tuple_p (list_t<pat_t>, bool ellipsis); // $(p1, ..., pn)
#endif
    Pointer_p(pat_t);  // &p
    Aggr_p(type_opt_t, bool tuple, list_t<tvar_t>, list_t<$(list_t<designator_t>, pat_t) @>,
           bool ellipsis);
    Datatype_p(datatypedecl_t, datatypefield_t, list_t<pat_t>, bool ellipsis);
    Null_p;                  // NULL
    Int_p(sign_t, int);      // 3
    Char_p(char);            // 'a'
    ...
    UnknownCall_p(qvar_t, list_t<pat_t>,
                  bool ellipsis);  // resolved by binding
  ...
  EXTERN_ABSYN struct Vardecl {
    ...
    attributes_t attributes;
    bool escapes;
    // set by type-checker -- used for simple
    ...
    bool is_proto;
    // used for suppression of gcc warnings ...
    // by inserting extern kw appropriately
    exp_opt_t rename;
    // optional asm expression,
  ...
  EXTERN_ABSYN struct Fndecl {
    ...
    scope_t
        orig_scope;  // after toc, this field is used to flag extern C functions ... ignore bodies
    bool escapes;
  };
  ...
  int varlist_cmp(list_t<var_t> a, list_t<var_t> b);
  ...
  // unsigned types
  extern type_t char_type, uchar_type, ushort_type, uint_type, ulong_type, ulonglong_type, uint128_type;
  // signed types
  extern type_t schar_type, sshort_type, sint_type, slong_type, slonglong_type, sint128_type;
  ...
  // misc constructors
  extern type_t void_type, ctor_type, var_type(tvar_t), tag_type(type_t),
      rgn_handle_type(rgntype_t), aqual_handle_type(type_t), aqual_var_type(type_t, type_t),
      aqual_constant(alias_qual_val_t), valueof_type(exp_t), typeof_type(exp_t),
#if 0
         access_eff (type_t),
#endif
      join_eff(types_t), regionsof_eff(type_t), enum_type(typedef_name_t, struct Enumdecl *`H),
      anon_enum_type(list_t<enumfield_t, `H>), builtin_type(string_t<`H>, kind_t),
  ...
  // t *{e}`r
  type_t starb_type(type_t, rgntype_t, aqualtype_t, tqual_t, ptrbound_t, booltype_t zero_term,
                    booltype_t rel);
  // t @{e}`r
  type_t atb_type(type_t, rgntype_t, aqualtype_t, tqual_t, ptrbound_t, booltype_t zero_term,
                  booltype_t rel);
  // t *`r (bounds = Upper(1)
  ...
  // t?`r
  type_t fatptr_type(type_t, rgntype_t, aqualtype_t, tqual_t, booltype_t zero_term, booltype_t rel);
  // structs
  ...
  // arrays
  type_t array_type(type_t elt_type, tqual_t, exp_opt_t num_elts, booltype_t zero_term,
                    seg_t ztloc);
  ...
  cnst_t Int128_c(sign_t, long long);
  ...
  exp_t assignop_exp(exp_t, primop_opt_t, exp_t, seg_t);
  ...
  struct AggrdeclImpl
      @aggrdecl_impl(list_t<tvar_t, `H> exists,
#if 0
                                      list_t<$(type_t, type_t) @`H, `H> po,
#endif
                     list_t<effconstr_t, `H> ec, list_t<$(type_t, type_t) @`H, `H> qb,
                     list_t<aggrfield_t, `H> fs, bool tagged);
  decl_t aggr_decl(aggr_kind_t, scope_t, typedef_name_t, list_t<tvar_t, `H>,
                   struct AggrdeclImpl *`H, attributes_t, seg_t);
  type_decl_t aggr_tdecl(aggr_kind_t, scope_t, typedef_name_t, list_t<tvar_t, `H>,
                         struct AggrdeclImpl *`H, attributes_t, seg_t);
  decl_t struct_decl(scope_t, typedef_name_t, list_t<tvar_t, `H>, struct AggrdeclImpl *`H,
                     attributes_t, seg_t);
  decl_t union_decl(scope_t, typedef_name_t, list_t<tvar_t, `H>, struct AggrdeclImpl *`H,
                    attributes_t, seg_t);
  decl_t datatype_decl(scope_t, typedef_name_t, list_t<tvar_t, `H>,
                       opt_t<list_t<datatypefield_t, `H>, `H>, bool is_extensible, seg_t);
  type_decl_t datatype_tdecl(scope_t, typedef_name_t, list_t<tvar_t, `H>,
                             opt_t<list_t<datatypefield_t, `H>, `H>, bool is_extensible, seg_t);

  type_t function_type(list_t<tvar_t, `H> tvs, type_opt_t eff_typ, tqual_t ret_tqual,
                       type_t ret_type, list_t<$(var_opt_t, tqual_t, type_t) @`H, `H> args,
                       bool c_varargs, vararg_info_t *`H cyc_varargs,
#if 0
                        list_t<$(type_t, type_t) @`H, `H> rgn_po,
#endif
                       list_t<effconstr_t, `H> effconstr, list_t<$(type_t, type_t) @`H, `H> qb,
                       attributes_t, exp_opt_t checks_clause, exp_opt_t requires_clause,
                       exp_opt_t ensures_clause, exp_opt_t throws_clause);
